...
function renderizarBuscadorMesa(mesa) {
    return `
        ...
        <div id="resultados-busqueda-${mesa.id}" style="display: none; background: white; border: 2px solid #667eea; border-radius: 8px; max-height: 200px; overflow-y: auto; margin-bottom: 12px;"></div>
    `;
}
...
function agregarProductosAMesa(mesaId, productos) {
    const mesa = mesasData[mesaId];
    if (!mesa) return;
