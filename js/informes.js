...
async function cargarInformes(modalBody) {
    ...
    `;
    try {
        // Obtener fechas por defecto (hoy - diario)
        const hoy = new Date();

        // Formato YYYY-MM-DD - Iniciar con el día de hoy (diario)
        const fechaInicio = hoy.toLocaleDateString('en-CA'); // Formato seguro ISO
...
function renderizarInformeConDatos(modalBody, datos, fechaInicio, fechaFin) {
    ...
    window.actualizarInputsYRefrescar = function (inicio, fin) {
        // Convertir Date objects a formato YYYY-MM-DD para los inputs
        const offset = inicio.getTimezoneOffset() * 60000;
        const fInicio = new Date(inicio.getTime() - offset).toISOString().split('T')[0];
        const fFin = new Date(fin.getTime() - offset).toISOString().split('T')[0];
    ...
    window.renderizarCalendarioVertical = function () {
        ...
        for (let i = -mesesAtras; i <= mesesAdelante; i++) {
            ...
            for (let dia = 1; dia <= diasEnMes; dia++) {
                ...
                // Usar formato de fecha consistente con zona horaria del tenant
    ...
    window.resaltarRangoVisual = function () {
        ...
        // Usar fechas directamente sin offset del navegador
        const inicio = window.rangoSeleccion.inicio.toLocaleDateString('en-CA');
        const fin = window.rangoSeleccion.fin.toLocaleDateString('en-CA');
    ...
    <div style="background: #f1f5f9; min-height: 100%;">
        ...
        <div class="report-content">
            ...
            <div style="text-align: center; margin-top: 30px; margin-bottom: 20px; font-size: 11px; color: #94a3b8;">
                <p style="margin: 0;">Generado: ${formatearFechaTenant(new Date().toISOString())}</p>
    ...
    setTimeout(() => {
        if (fechaInicio && fechaFin) {
            ...
            const label = document.getElementById('rango-fechas-label');
            if (label) label.textContent = (t1 === t2) ? t1 : `${t1} - ${t2}`;
        }
    }, 100);
}
